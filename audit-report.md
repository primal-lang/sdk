### 2. Static recursion depth counter not thread-safe (documented limitation)

**File**: `/home/max/Repositories/personal/primal-sdk/lib/compiler/runtime/node.dart`
**Lines**: 318-319

- **Issue**: `FunctionNode._currentDepth` is a static mutable field. While the code comments document this as intentional for single-threaded use, if someone were to use the runtime in an isolate-per-request pattern, they could experience incorrect recursion limit enforcement.
- **Impact**: In multi-isolate scenarios (not currently supported), recursion limits could be bypassed or triggered incorrectly. This is documented behavior, so flagged as warning rather than critical.
- **Fix**: Consider making this isolate-local or documenting this limitation more prominently in the public API.

**Follow-up**:

- **Tests**: Current tests verify single-threaded behavior; consider adding documentation tests
  - Success case: Recursive function hitting depth 1000 throws `RecursionLimitError`
  - Failure case: N/A (current behavior is correct for documented use case)
- **Docs**: Consider adding note to public API documentation about single-threaded assumption

---

### 3. VectorAdd error message reports only one type when both are wrong

**File**: `/home/max/Repositories/personal/primal-sdk/lib/compiler/library/vector/vector_add.dart`
**Line**: 47

- **Issue**: When both arguments are not `VectorNode`, the error only reports `[a.type]` instead of both types.
- **Impact**: Confusing error messages when calling `vector.add` with incorrect types.
- **Fix**:
...
throw InvalidArgumentTypesError(
  ...
  actual: [a.type, b.type],  // Changed from [a.type]
);
```

**Follow-up**:

- **Tests**: Add test for `vector.add` with non-vector arguments
  - Success case: `vector.add(vector.new([1,2]), vector.new([3,4]))` returns correct result
  - Failure case: `vector.add("a", "b")` produces error listing both types
