...
- **Issue**: When `a > b` or when the range is very large, the expression `(b.value - a.value + 1).toInt()` can be negative or overflow. `Random.nextInt()` throws `ArgumentError` for non-positive values.
- **Impact**: Runtime crash with an internal Dart error instead of a meaningful Primal error message.
- **Fix**:
