...
- **Signature:** `json_encode(a: Any): String`
...
- **Signature:** `json_decode(a: String): Any`
- **Input:** A JSON string.
...
> **Note:** JSON `null` values are not supported. When decoding, `null` values in objects are skipped (the key is omitted), and `null` values in arrays are filtered out. A top-level `null` (e.g., `json_decode("null")`) throws a runtime error.
