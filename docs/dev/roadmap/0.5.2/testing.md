...
sources:
  ...
  - lib/compiler/library/logic/bool_and_strict.dart
  - lib/compiler/library/operators/operator_double_and.dart
  - lib/compiler/library/set/set_new.dart
  - lib/compiler/errors/generic_error.dart
  ...
  - test/helpers/console_fakes.dart
---
...
code ([§4.8](#48-runcli-return-codes)). Two existing declarations also move
without changing behaviour: `RuntimeError` gains an optional `category`
parameter, and `Runtime.format` becomes `static` so a native can reach it
...
([§4.3](#43-assertion-functions)), so the short-circuit branch is unreachable and
strict `&` would chain them identically **in every case, including failure**.
`BoolAndStrict.execute` reduces `a` before `b`
(`lib/compiler/library/logic/bool_and_strict.dart`), so under `&` the first
assertion to throw also prevents the rest from running — that is Dart exception
propagation, not short-circuiting. A test reports one failure at a time under
either operator; `&&` is specified only because it is the conventional spelling.
...
- **The REPL path never returns in production — but it does under test.**
  `_runRepl` calls `Console.prompt`, which is
  `while (true) { promptOnce(handler); }` (`lib/utils/console.dart`), so the real
  process leaves only through `exit(0)`. The fake is different:
  `ScriptedConsole.prompt` replaces the loop with a bounded `for`
  (`test/helpers/console_fakes.dart`), so the REPL path returns in every one of
  the ~60 tests in `test/compiler/main_cli_test.dart` that take it. The return
  value is observable and must be specified. Dart forces the issue anyway: the
  body of an `int runCli(...)` may not complete normally, so "unreachable" is not
  an option the compiler accepts ([§4.8](#48-runcli-return-codes)).
...
Add two runtime error types: `AssertionFailedError`, and `AssertionArgumentError`
for an assertion that is *misused* rather than unsatisfied
([§6.9](#69-nested-assertion-misuse-inside-assertthrows)). Promote
`Runtime.format` to `static` so the natives can render values
...
5. An `InvalidArgumentTypesError` raised by the comparison in step 2 — from the
   top-level kind check *or* from the recursive element comparison — is caught
   and rethrown as `AssertionArgumentError`, preserving its message verbatim
   ([§4.4](#44-failure-representation)). The rendered text is identical; the
   wrapper exists solely so `assert.throws` can refuse to absorb it
   ([`assert.throws`](#assertthrowsa) below).
6. Any other error propagates **unchanged**, whether it is raised while
   *reducing* either operand in step 1 or by the comparison in step 2. Both are
   test **errors**, not failures: `assert.equal(to.number("x"), 1)` reports the
...
5. An `InvalidArgumentTypesError` from the comparison is rewrapped as
   `AssertionArgumentError`, and any other error propagates **unchanged**, from
   operand reduction or from the comparison — exactly as for `assert.equal`.
...
2. If it is not a `BooleanTerm`, throw `AssertionArgumentError` wrapping an
   `InvalidArgumentTypesError` of the same shape as `if`'s
   (`lib/compiler/library/control/if.dart`): `function: name`,
   `expected: parameterTypes`, `actual: [a.type]`. It renders exactly like a
   plain `InvalidArgumentTypesError` ([§4.4](#44-failure-representation)), so
   [§5.2](#52-invalid-with-expected-results)'s output is unaffected.
...
| throws `AssertionArgumentError`                                   | rethrow **unchanged** — a nested assertion misuse is not "a throw"     |
...
`AssertionArgumentError` is the reason the second row exists. Without it
`assert.throws(assert.true(1))` would **pass**: the inner helper raises a plain
`InvalidArgumentTypesError`, which is an ordinary `RuntimeError`, so the
catch-all row would absorb it and report a broken assertion as green — the exact
failure mode [§3.3](#33-assertthrows-is-narrower-than-try) rejects `try`'s
catch-all for, re-entering through a different door. The distinction cannot be
drawn by error type alone, because a *legitimate* expectation like
`assert.throws(num.add(1, "x"))` must still pass on an `InvalidArgumentTypesError`
raised by the code under test. See [§6.9](#69-nested-assertion-misuse-inside-assertthrows).

**Implementation constraint — the guarded region must not enclose its own
failure.** `AssertionFailedError`, `AssertionArgumentError` and
`RecursionLimitError` are all `RuntimeError` subclasses, so the two obvious
...
- Ordering `on RuntimeError` before the narrower clauses swallows all three
...
AssertionFailedError`, `on AssertionArgumentError` and `on RecursionLimitError`
must precede `on RuntimeError`. Non-`RuntimeError` throwables are not caught at
all, so no clause is needed for them.
...
AssertionFailedError extends RuntimeError
  ...
  constructor:  ({required String function,
...
AssertionArgumentError extends RuntimeError
  category:     'Runtime error'  (inherited — an assertion misused is an error,
                                  not a failure, and must not read as one)
  constructor:  (InvalidArgumentTypesError cause) : super(cause.message)
  renders:      identically to the InvalidArgumentTypesError it wraps
```
...
message body of `AssertionFailedError` deliberately omits the word "Assertion":
the category already supplies it, and `Assertion error: Assertion "…" failed`
reads as a stutter.

`AssertionArgumentError` carries **no** new message template. It wraps the
`InvalidArgumentTypesError` a helper would otherwise have thrown and forwards
`cause.message` — `message` is a public field on `GenericError`
(`lib/compiler/errors/generic_error.dart`). Rendering is therefore byte-identical
to today's, which is why [§5.2](#52-invalid-with-expected-results)'s expected
output needs no change; the type exists only to be recognisable to
`assert.throws` ([§4.3](#43-assertion-functions)). Forwarding the message rather
than rebuilding it also preserves the *inner* operand types on the recursive
collection path: `assert.equal([1], ["x"])` keeps reporting
`Actual: (Number, String)` rather than degrading to `(List, List)`.

The three constructor arguments of `AssertionFailedError` are **not** stored as
fields. Every other `RuntimeError` subclass keeps only the composed message
(`lib/compiler/errors/runtime_error.dart`; only `CustomError` retains a payload,
and even that never reaches output), and nothing in this specification reads them
back — the runner prints `error.toString()`
([§4.7](#47-cli-test-runner)). All five helpers supply both values, so neither
...
**Value rendering — decided, and it requires one change to `Runtime`.**
`Runtime.format` is today an **instance** method on a class constructed from a
`RuntimeInput` (`lib/compiler/runtime/runtime.dart`), reachable only through
`RuntimeFacade.evaluate` and `RuntimeFacade.format`
(`lib/compiler/lowering/runtime_facade.dart`). Native function terms live in
`lib/compiler/library/**` and see nothing but `Term`s
(`NativeFunctionTermWithArguments`, `lib/compiler/runtime/term.dart`); they have
no `Runtime` handle and no route to one, so the assertion helpers cannot call it
as written.

**`Runtime.format` therefore becomes `static`.** The method never touches
`Runtime.input`, so the promotion is mechanical: the two existing call sites in
`RuntimeFacade` change from `_runtime.format(...)` to `Runtime.format(...)`.
[§7](#7-compiler-impact-by-stage) records it. The alternatives were rejected —
storing raw `Term`s and rendering in the CLI runner would leave
`AssertionFailedError.toString()` unable to describe itself in the REPL, on the
web target, or anywhere the error is caught by `try`; duplicating the quoting
rules inside an assert helper guarantees drift the next time a type is added.

A reduced `Term` is then turned into its `String` with
`Runtime.format(term.native()).toString()` — the same renderer the CLI already
uses to print a program's result. `Term.toString()` must **not** be used: `ValueTerm`
...
`assert.throws` and the `not `-prefixed `expected` of `assert.notEqual`. The
`expected` of `assert.throws` is the fixed string `a thrown error`, not a
rendered value, and is inserted literally.

**Rendering must not be able to escape.** Two paths inside
`Runtime.format(term.native())` can throw: `format` raises `InvalidValueError`
for a value it does not recognise (`lib/compiler/runtime/runtime.dart`), and
`native()` raises `StateError` on an unsubstituted `BoundVariableTerm`,
`LetBoundVariableTerm` or `LambdaBoundVariableTerm`
(`lib/compiler/runtime/term.dart`). Left unguarded, a failing assertion could
throw a `StateError` while building its own message, which
[§4.7](#47-cli-test-runner) classifies as an **abort** — one bad value would end
the whole run. Each helper therefore renders inside a guard and falls back to
`term.toString()` if rendering throws. The fallback is lossy (it drops string
quotes) but bounded, and it keeps the failure a **fail**.

(The first draft declared both values `String?` without defining what the message
...
| Condition                                         | Result                                                                            |
| ------------------------------------------------- | --------------------------------------------------------------------------------- |
| `assert.true` / `assert.false` with a non-boolean | `AssertionArgumentError` (rendering as today's `InvalidArgumentTypesError`) → test **error** |
| `assert.equal` across incomparable types          | `InvalidArgumentTypesError` from `CompEq.execute`, reported against `assert.equal`, rewrapped as `AssertionArgumentError` → test **error** |
| `assert.equal` over equal-length lists, vectors, stacks or queues with mismatched element kinds | same, from the recursive element comparison → test **error** |
| `assert.equal` over maps with mismatched **value** kinds | same, from the recursive value comparison → test **error**                  |
...
| `assert.notEqual` across incomparable types       | `AssertionArgumentError` — `CompNeq.execute` delegates to `CompEq.execute` → test **error** |
...
| `assert.throws` over a nested *misused* assertion (`assert.throws(assert.true(1))`) | `AssertionArgumentError` rethrown unchanged → test **error** ([§6.9](#69-nested-assertion-misuse-inside-assertthrows)) |
| `assert.throws` over a type error raised by the code under test (`assert.throws(num.add(1, "x"))`) | plain `InvalidArgumentTypesError` absorbed → test **pass** |
...
| user function named exactly `assert.equal`, `assert.notEqual`, `assert.true`, `assert.false` or `assert.throws` in a source file | `DuplicatedFunctionError` at compile time |
| the same names in the REPL                        | `CannotRedefineStandardLibraryError`                                              |
| user function named `assert.somethingElse`        | **legal** — collision detection compares whole names (`SemanticAnalyzer.analyze`), and unlike `test.`, the `assert.` prefix is *not* reserved ([§4.7](#47-cli-test-runner)) |
...
- **The runner owns its own error boundary.** Reading and compiling the file must
  happen inside a `try` belonging to the test runner, not inside the pre-existing
  catch-all of `runCli` (`lib/main/main_cli.dart`). That catch-all returns `1`
  ([§4.8](#48-runcli-return-codes)); a failure to *build* the file under `--test`
  returns `2`, because the run did not measure what it claimed to. Without a
  dedicated boundary the two rules silently collide and `primal --test broken.prm`
  exits `1`. The boundary covers the file read as well as compilation, so a
  missing or unreadable path — `FileReader.read` throws before the compiler is
  reached — is also `2`.
...
- The `test.` prefix is **reserved**: the standard library must never register a
  ...
  ([§4.6](#46-error-conditions)). The reservation runs in that one direction only,
  and it is **not** extended to `assert.`: users may define `assert.somethingElse`
  today and still may afterwards, because collision detection compares whole
  names, not prefixes ([§4.6](#46-error-conditions)).
...
`error` for everything on stderr. No new `Console` method is required. Two
consequences of reusing `error`: stderr lines are red, and they carry no
`errorType:` prefix of their own — every other stderr line in the CLI gets one
from `GenericError.toString()` — so the runner writes the literal `Error: ` at
the start of each such message itself.

**The two streams are not ordered relative to each other.** `Console.print` and
`Console.error` write to independently buffered sinks, so the transcript a
terminal shows may interleave stdout and stderr in any order. The listings below
are grouped for readability and must not be read as an ordering guarantee;
assertions in [§10](#10-post-implementation) are per-stream for the same reason.
...
- The "returned a non-`true` value" case has no error object, so the runner
  synthesises its second line. The exact text is
  `test "<name>" did not return true (returned <value>)`, with `<value>` rendered
  by `Runtime.format` ([§4.4](#44-failure-representation)).
- The summary counts every discovered test — **including skipped ones** — and
  omits the categories that are zero, always keeping the count and at least one
  category. Categories are `passed`, `failed`, `error` and `skipped`, in that
  order. The count reads `1 test:` when it is exactly one and `N tests:`
  otherwise.
- A skipped test is counted but has no per-test line on stdout; its report is the
  stderr line described under *Discovery*. Counting it is what stops the summary
  from reading fully green on a run that exits `2`
  ([§5.3](#53-invalid-test-mode-example)).
- With `--debug`, each per-test line gains a trailing `[Nms]` measured around the
  `evaluateToTerm` call. The indented error line and the summary do not. The
  compile timing is printed first, reusing the existing
  `[debug] Compilation: Nms` format verbatim (`lib/main/main_cli.dart`).
...
| `2`  | usage error, unreadable file, compile error, no tests discovered, a skipped `test.*`, or an internal abort |

The `2` cases that arise before any test runs — unreadable file and compile error
— reach this table only because the runner installs its own error boundary around
the read and the compile (*Discovery* above). Left to `runCli`'s catch-all they
would return `1` ([§4.8](#48-runcli-return-codes)).
...
| REPL                                          | `0`. Unreachable in production, but reached under `ScriptedConsole` ([§2.9](#29-the-cli-has-no-exit-code-contract)) and required by the compiler in any case. A session whose last input raised an error still returns `0`: `_runRepl` swallows errors per input in its own `catch` (`lib/main/main_cli.dart`), so no error survives to the return |
| the `catch` in `runCli` (compile or runtime failure, **non-test runs only**) | `1`               |
...
It applies to non-test runs only. Under `--test` the read and the compile sit
inside the runner's own boundary ([§4.7](#47-cli-test-runner)), which returns `2`
before the catch-all is reached — so `1` keeps meaning "the program under test
misbehaved" and never "the file would not build".
...
```text
primal --test sample.prm
Error: skipped "test.helper" — test functions must take no parameters   (stderr)
PASS  test.real                                                          (stdout)

2 tests: 1 passed, 1 skipped                                             (stdout)
→ exit code 2
```

The skipped function is counted, so the summary cannot read fully green on a run
that exits `2` ([§4.7](#47-cli-test-runner)). The stream annotations are
illustrative — stdout and stderr are separately buffered and their relative order
is not defined.
...
`try`-equivalent catch-all this would incorrectly pass. The companion case — a
nested assertion that is *misused* rather than unsatisfied — is
[§6.9](#69-nested-assertion-misuse-inside-assertthrows); it needs a separate
mechanism, because a type error is indistinguishable by class from one the test
...
assert.equal(set.new([1]), set.new(["x"]))          // FAIL  — sets never delegate to comp.eq
```

Sets are built with `set.new` (`lib/compiler/library/set/set_new.dart`), not with
a literal: `{` opens a **map** in the grammar, so `{1}` is a syntax error
(`Expected: :`). Sets only *print* as `{1}`, which is why the shorthand is
tempting and wrong.
...
assert.equal(set.new([1]), set.new(["x"]))          // FAIL
```
...
- Set membership uses Dart equality on native values, so sets of collections
  compare by identity: `assert.equal(set.new([[1]]), set.new([[1]]))` **fails**
  even though the elements are structurally equal.
...
### 6.9 Nested Assertion Misuse Inside `assert.throws`

```primal
test.a() = assert.throws(assert.true(1))          // ERROR, not pass
test.b() = assert.throws(assert.equal("1", 1))    // ERROR, not pass
test.c() = assert.throws(num.add(1, "x"))         // PASS — a genuine expectation
```

[§6.1](#61-nested-assertion-inside-assertthrows) covers a nested assertion that
*fails*. This is the neighbouring case: a nested assertion that is *misused*.

`assert.true(1)` does not fail, it raises a type error, and a type error is an
ordinary `RuntimeError` (`lib/compiler/errors/runtime_error.dart`). Under the
catch-all row of [§4.3](#43-assertion-functions) it would be absorbed and
`test.a` would report **pass** — a broken assertion reported green, which is
exactly what [§3.3](#33-assertthrows-is-narrower-than-try) declines `try`'s
catch-all in order to prevent.

The distinction cannot be drawn on error type alone, because `test.c` raises the
*same* Dart class and must still pass: there, the type error is the behaviour
under test. What separates them is **who raised it**, and errors carry no origin
([§2.4](#24-errors-carry-no-inspectable-payload)). Hence
`AssertionArgumentError` ([§4.4](#44-failure-representation)): the assertion
helpers, and only they, raise the wrapper, and `assert.throws` rethrows it
unchanged. Rendering is unaffected, so a misused assertion outside
`assert.throws` prints exactly as it does today.
...
| Lowering  | Assertions lower as ordinary native calls and tests lower like any other custom function, but `RuntimeFacade`'s two `_runtime.format(...)` call sites become `Runtime.format(...)` as a consequence of the promotion below ([§4.4](#44-failure-representation)). Mechanical; no behaviour change. |
| Runtime   | Five new natives following the two-class pattern, with parameters named `a`, `b` per convention ([§4.3](#43-assertion-functions)), plus `AssertionFailedError` and `AssertionArgumentError` ([§4.4](#44-failure-representation), [§6.9](#69-nested-assertion-misuse-inside-assertthrows)). `assert.throws` reduces its argument under a guarded region whose failure path lies outside it ([§4.3](#43-assertion-functions)). `RuntimeError` gains an optional `category` parameter (default-valued, so existing subclasses are untouched) so failures print as `Assertion error` ([§4.4](#44-failure-representation)). **`Runtime.format` becomes `static`** so the natives can reach it — it never touches `Runtime.input`, so the promotion is mechanical, but it is a change to a core class rather than a pure addition ([§4.4](#44-failure-representation)). |
| CLI       | New `--test` / `-t` mode; discovery, execution, classification, and reporting ([§4.7](#47-cli-test-runner)), including the runner's **own** error boundary around the file read and compilation so build failures return `2` rather than falling into `runCli`'s catch-all. `runCli` returns `int` and `main` assigns `exitCode` for **every** path, which changes the exit code of failing non-test runs from `0` to `1` ([§4.8](#48-runcli-return-codes)). |
...
- **`assert.throws` stays narrower than `try`** — confirmed. It catches
  `RuntimeError` only, rethrowing `AssertionFailedError`,
  `AssertionArgumentError`, `RecursionLimitError` and any non-`RuntimeError`
  unchanged ([§3.3](#33-assertthrows-is-narrower-than-try),
  [§4.3](#43-assertion-functions)).
- **A misused nested assertion is an error, not a caught throw** — confirmed.
  `assert.true(1)` raises a type error, not a failure, and a type error is an
  ordinary `RuntimeError`; absorbing it would let `assert.throws(assert.true(1))`
  report green. It cannot be told apart from a legitimate
  `assert.throws(num.add(1, "x"))` by class alone, so the assertion helpers wrap
  their own type errors in `AssertionArgumentError` and `assert.throws` rethrows
  that. Rendering is unchanged, so no existing expected output moves
  ([§4.4](#44-failure-representation),
  [§6.9](#69-nested-assertion-misuse-inside-assertthrows)).
...
- **Values render through `Runtime.format`, which becomes `static`** —
  confirmed. `Term.toString()` drops quotes on strings, which would make
  `expected 2, actual 3` ambiguous and defeat the reason for auto-generating
  messages at all. `format` is an instance method today and unreachable from a
  native, so it is promoted to `static`; it never touches `Runtime.input`, and
  the only call sites are two lines in `RuntimeFacade`. Storing raw `Term`s and
  rendering in the CLI runner was rejected because the error would then be unable
  to describe itself in the REPL, on the web target, or when caught by `try`
  ([§4.4](#44-failure-representation), [§7](#7-compiler-impact-by-stage)).
- **Rendering failures are contained** — confirmed. `Runtime.format` can raise
  `InvalidValueError` and `native()` can raise `StateError`, either of which
  would turn a failing assertion into a run-ending abort. Each helper renders
  under a guard and falls back to `term.toString()`
  ([§4.4](#44-failure-representation), [§4.7](#47-cli-test-runner)).
...
- **`runCli` gets a return code for every path, and failing runs now exit `1`** —
  confirmed. Watch mode returns `0` early, the catch-all returns `1`, and the
  REPL returns `0` — *not* "unreachable": `ScriptedConsole` gives `prompt` a
  bounded loop, so the tests reach that return, and Dart requires a value there
  regardless. The catch-all change is a deliberate breaking change: a CLI that
  exits `0` on a compile error is unusable in CI
  ([§2.9](#29-the-cli-has-no-exit-code-contract), [§4.8](#48-runcli-return-codes)).
- **A build failure under `--test` exits `2`, not `1`** — confirmed, which
  requires the runner to own an error boundary around the file read and the
  compile. Left to `runCli`'s catch-all it would return `1`, and `2` would stop
  meaning "the run did not measure what it claimed to" for the single most common
  way an invocation goes wrong ([§4.7](#47-cli-test-runner),
  [§4.8](#48-runcli-return-codes)).
- **The runner's output format is fixed** — confirmed. `PASS`/`FAIL`/`ERROR`
  ...
  aborts on stderr, whose order relative to stdout is not defined
  ([§4.7](#47-cli-test-runner)).
- **Skipped tests are counted in the summary** — confirmed. `2 tests: 1 passed,
  1 skipped` rather than `1 test: 1 passed`. The summary is what a human reads,
  and letting it report fully green on a run that exits `2` reintroduces exactly
  the invisibility that skipping loudly was meant to remove
  ([§4.7](#47-cli-test-runner), [§5.3](#53-invalid-test-mode-example),
  [§6.5](#65-test-with-parameters)).
...
- **A test must return exactly `true` to pass** — confirmed, on narrower grounds
  than first stated. It catches a test whose final expression is a *non-boolean*
  — `test.bad() = 42` is an error rather than a silent pass. It does **not**
  catch a test with no assertions at all: `test.x() = true` and
  `test.x() = 1 == 1` both reduce to `BooleanTerm(true)` and pass. Nothing in the
  first version detects an assertion-free test, and the rule should not later be
  "fixed" to claim otherwise. The cost is the extra "returned a non-`true` value"
  classification in [§4.7](#47-cli-test-runner), and that a test whose final
  expression is a side effect must end in an assertion.
- **The `test.` prefix is reserved; `assert.` is not** — confirmed. The standard
  library must never claim `test.` ([§4.7](#47-cli-test-runner)). The `assert.`
  namespace gets no such protection in either direction: `SemanticAnalyzer.analyze`
  compares whole names, so only the five exact names collide, and
  `assert.somethingElse()` is legal today and stays legal
  ([§4.6](#46-error-conditions)). A second discovery prefix and any
...
- **`AssertionFailedError` prints under its own category** — confirmed.
  ...
  ([§4.4](#44-failure-representation), [§7](#7-compiler-impact-by-stage)).
  `AssertionArgumentError` deliberately does **not** take one: an assertion
  misused is an error, and must not read as a failure
  ([§6.9](#69-nested-assertion-misuse-inside-assertthrows)).
- **`AssertionFailedError` stores no fields** — confirmed. `function`, `actual`
  and `expected` are constructor arguments folded into the message, not retained
  state. Nothing reads them back — the runner prints `error.toString()` — and
  every sibling in `lib/compiler/errors/runtime_error.dart` keeps only the
  composed message ([§4.4](#44-failure-representation)).
...
- Cross-reference `AssertionFailedError` and `AssertionArgumentError` from
  [[lang/reference/core/error]].
...
- Update [[dev/architecture/error/error-hierarchy]] with the two new error types
  **and** with `RuntimeError`'s optional `category` parameter — the hierarchy is
  no longer "one category per base class". `AssertionArgumentError` needs its own
  note: it is a wrapper whose whole purpose is recognisability, and it renders
  identically to what it wraps ([§4.4](#44-failure-representation),
  [§6.9](#69-nested-assertion-misuse-inside-assertthrows)).
- Update [[dev/architecture/runtime/native-functions]] with `Runtime.format`'s
  promotion to `static`, since it is the mechanism by which a native reaches the
  shared renderer ([§4.4](#44-failure-representation)).
...
- `assert.throws` and nested misuse, as a matched pair
  ([§6.9](#69-nested-assertion-misuse-inside-assertthrows)):
  `assert.throws(assert.true(1))` and `assert.throws(assert.equal("1", 1))` are
  **errors**, while `assert.throws(num.add(1, "x"))` **passes**. One without the
  other proves nothing — the first pair guards against absorbing a broken
  assertion, the second against over-correcting into rejecting a legitimate
  expectation.
- `AssertionArgumentError.toString()` is byte-identical to the
  `InvalidArgumentTypesError` it wraps, so the messages in
  [§5.2](#52-invalid-with-expected-results) survive unchanged.
- `assert.equal`: cross-type propagation, unequal-length collections,
  ...
  ([§6.4](#64-collection-comparison-splits-four-ways)); `1` versus `1.0`. Set
  cases are written with `set.new([…])`, not `{…}`, which is map syntax.
- `Runtime.format`'s promotion to `static` changes no output: the existing
  `RuntimeFacade.evaluate` rendering tests are the regression guard, and must
  pass untouched ([§4.4](#44-failure-representation)).
- A value whose rendering throws produces a **fail** with the `toString()`
  fallback, not an abort ([§4.4](#44-failure-representation)).
...
- All three classifications plus the non-`true` return case, asserted against the
  ...
  prefix, the indented error line, the exact
  `test "<name>" did not return true (returned <value>)` text, and the summary
  counts.
- A file combining a skipped `test.*` with passing tests summarises as
  `2 tests: 1 passed, 1 skipped` and exits `2` — the guard that the summary
  cannot read green on a run the exit code calls invalid
  ([§5.3](#53-invalid-test-mode-example)).
- Singular and plural summaries: `1 test: 1 passed` and `3 tests: 3 passed`.
- A compile error under `--test` returns **`2`**, not `1` — the guard that the
  runner's error boundary exists and that `runCli`'s catch-all did not swallow
  the case ([§4.7](#47-cli-test-runner), [§4.8](#48-runcli-return-codes)). Same
...
- A test returning the *string* `"true"` is an **error**, not a pass — the guard
  ...
  ([§4.7](#47-cli-test-runner)).
- Results appear on stdout, skips and aborts on stderr — asserted **per stream**,
  never against a single interleaved transcript, since the two sinks are
  independently buffered and their relative order is undefined
  ([§4.7](#47-cli-test-runner)).
...
- Every row of the `runCli` table in [§4.8](#48-runcli-return-codes), including
  the changed one: a non-test run whose program fails now returns `1`. The REPL
  row is directly testable through `ScriptedConsole` and must assert `0`,
  including for a session whose last input raised an error; watch mode asserts
  the early `0`.
...
- A user function named exactly `assert.equal` in a source file fails compilation
  with `DuplicatedFunctionError` (not `CannotRedefineStandardLibraryError`), and
  — as the counter-case — `assert.somethingElse()` still compiles, since only
  whole names collide ([§4.6](#46-error-conditions)).

## 11. Implementation Complexity

**Low to medium.**

The lexer, parser and semantic analyzer need no changes at all, and argument
laziness comes free with the native calling convention. Each helper is a short
two-class native, plus two error classes. Lowering is touched only by two
mechanical call-site edits in `RuntimeFacade`
([§7](#7-compiler-impact-by-stage)).

Three pieces carry the real risk, and none is large:
...
- **The exit-code contract.** Threading an `int` out of `runCli` without calling
  ...
  it is where the design leaks. The specific trap is that `--test` needs its own
  error boundary — dropped into the existing `try`, every build failure returns
  `1` and the `2` contract quietly stops holding
  ([§4.7](#47-cli-test-runner)).
- **`assert.throws`'s guarded region.** The natural transcription is wrong and
  ...
  the `assert.throws(42)` test is not optional. The same clause ordering also
  decides whether a *misused* nested assertion is absorbed, which is the harder
  half to notice ([§6.9](#69-nested-assertion-misuse-inside-assertthrows)).
- **Reaching the renderer.** `Runtime.format` is an instance method today, so the
  helpers cannot call it until it is promoted to `static`. The promotion is
  trivial; discovering the need for it halfway through writing the first helper
  is what costs the time ([§4.4](#44-failure-representation)).
