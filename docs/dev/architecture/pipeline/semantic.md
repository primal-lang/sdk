...
**TLDR**: The semantic analyzer validates function definitions through a two-pass process (signature extraction, then identifier resolution and arity checking), producing an intermediate representation that is then lowered to runtime terms for evaluation.
