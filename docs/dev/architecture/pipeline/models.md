...
**TLDR**: Core data types shared across all compiler phases, including `Location` for source positions, `Parameter` for function parameters with type constraints, `FunctionSignature` for phase-agnostic call validation, and `Analyzer` as the base class for pipeline stages.
