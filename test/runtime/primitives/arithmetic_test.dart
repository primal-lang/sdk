...
void main() {
  ...
  group('Numeric Edge Cases', () {
    ...
    test('num.log(0) returns negative infinity', () {
      final Runtime runtime = getRuntime('main = num.log(0)');
      checkResult(runtime, double.negativeInfinity);
    });

    test('num.pow(0, 0) returns 1', () {
      final Runtime runtime = getRuntime('main = num.pow(0, 0)');
      checkResult(runtime, 1);
    });

    test('num.sin(0) returns 0', () {
      final Runtime runtime = getRuntime('main = num.sin(0)');
      checkResult(runtime, 0.0);
    });

    test('num.cos(0) returns 1', () {
      final Runtime runtime = getRuntime('main = num.cos(0)');
      checkResult(runtime, 1.0);
    });

    test('num.tan(0) returns 0', () {
      final Runtime runtime = getRuntime('main = num.tan(0)');
      checkResult(runtime, 0.0);
    });

    test('num.abs(0) returns 0', () {
      final Runtime runtime = getRuntime('main = num.abs(0)');
      checkResult(runtime, 0);
    });

    test('num.negative(0) returns 0', () {
      final Runtime runtime = getRuntime('main = num.negative(0)');
      checkResult(runtime, 0);
    });

    test('num.fraction(0) returns 0', () {
      final Runtime runtime = getRuntime('main = num.fraction(0)');
      checkResult(runtime, 0);
    });

    test('num.isEven(0) returns true', () {
      final Runtime runtime = getRuntime('main = num.isEven(0)');
      checkResult(runtime, true);
    });

    test('num.isOdd(0) returns false', () {
      final Runtime runtime = getRuntime('main = num.isOdd(0)');
      checkResult(runtime, false);
    });

    test('large exponent', () {
      final Runtime runtime = getRuntime('main = num.pow(2, 32)');
