...
void main() {
  group('UUID V4', () {
    test('uuid_v4 returns a string of length 36', () {
      final RuntimeFacade runtime = getRuntime(
        'main() = str_length(uuid_v4())',
      );
      checkResult(runtime, 36);
    });

    test('uuid_v4 returns a quoted string', () {
      final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
    ...
    test('uuid_v4 has correct format with dashes', () {
      final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
      final String result = runtime.executeMain();
    ...
    test('uuid_v4 has version 4 indicator', () {
      final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
      final String result = runtime.executeMain();
      final String uuid = result.substring(1, result.length - 1);
    ...
    test('uuid_v4 has correct variant bits', () {
      final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
      final String result = runtime.executeMain();
      final String uuid = result.substring(1, result.length - 1);
    ...
    test('uuid_v4 contains only valid hex characters and dashes', () {
      final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
      final String result = runtime.executeMain();
      final String uuid = result.substring(1, result.length - 1);
    ...
    test('uuid_v4 generates unique values', () {
      final RuntimeFacade runtime1 = getRuntime('main() = uuid_v4()');
      final RuntimeFacade runtime2 = getRuntime('main() = uuid_v4()');
      final String result1 = runtime1.executeMain();
      final String result2 = runtime2.executeMain();
      expect(result1, isNot(equals(result2)));
    });

    test('uuid_v4 multiple calls return different values', () {
      final RuntimeFacade runtime = getRuntime(
        'main() = list_distinct([uuid_v4(), uuid_v4(), uuid_v4()])',
      );
      final String result = runtime.executeMain();
      // If all three are unique, list_distinct returns a list of 3 elements
    ...
    test('uuid_v4 result can be used in string operations', () {
      final RuntimeFacade runtime = getRuntime(
        'main() = str_contains(uuid_v4(), "-")',
      );
      checkResult(runtime, true);
    });

    test('uuid_v4 result can be compared as not equal', () {
      final RuntimeFacade runtime = getRuntime(
        'main() = comp_neq(uuid_v4(), uuid_v4())',
      );
      checkResult(runtime, true);
    });

    test('uuid_v4 matches RFC 4122 v4 pattern', () {
      final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
      final String result = runtime.executeMain();
      final String uuid = result.substring(1, result.length - 1);
    ...
    test('uuid_v4 can be concatenated with strings', () {
      final RuntimeFacade runtime = getRuntime(
        'main() = str_startsWith(str_concat("id:", uuid_v4()), "id:")',
      );
      checkResult(runtime, true);
    });

    test('uuid_v4 result has correct total length including quotes', () {
      final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
      final String result = runtime.executeMain();
  ...
  group('UUID V4 Consistency', () {
    test('uuid_v4 generates many valid UUIDs', () {
      for (int i = 0; i < 100; i++) {
        final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
        final String result = runtime.executeMain();
        final String uuid = result.substring(1, result.length - 1);
    ...
    test('uuid_v4 always produces lowercase hex', () {
      for (int i = 0; i < 50; i++) {
        final RuntimeFacade runtime = getRuntime('main() = uuid_v4()');
        final String result = runtime.executeMain();
        final String uuid = result.substring(1, result.length - 1);
