...
void main() {
  group('Lazy Evaluation', () {
    ...
    test('try preserves result type from primary', () {
      final RuntimeFacade runtime = getRuntime('main = try("hello", "world")');
...
''');
  ...
  group('Error Handling in Lazy Evaluation', () {
    ...
    test('bool.or throws when first argument is not boolean', () {
      final RuntimeFacade runtime = getRuntime('main = bool.or("yes", false)');
      expect(runtime.executeMain, throwsA(anything));
    });

    test('bool.or throws when second argument is not boolean', () {
      final RuntimeFacade runtime = getRuntime('main = bool.or(false, "yes")');
      expect(runtime.executeMain, throwsA(anything));
    });
  ...
  group('Edge Cases', () {
    ...
    test('if with empty string as result', () {
      final RuntimeFacade runtime = getRuntime('main = if (true) "" else "x"');
      checkResult(runtime, '""');
    });

    test('if with empty list as result', () {
      final RuntimeFacade runtime = getRuntime('main = if (true) [] else [1]');
      checkResult(runtime, '[]');
    });
